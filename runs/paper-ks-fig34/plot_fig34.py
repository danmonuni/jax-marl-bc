...
steps, all agents) and writes ``3.png`` and ``4.png``.
    ...
    python runs/paper-ks-fig34/plot_fig34.py out_dir=<dir>      # write elsewhere
    python runs/paper-ks-fig34/plot_fig34.py run=<dir> window=50

The conventions, and how they differ from an earlier generation of these two
figures:
...
HERE = Path(__file__).resolve().parent          # runs/paper-ks-fig34/
...
#: House style: jmbc's own style with savefig.dpi raised to 300 for print and
#: legend text set at the axis-label size rather than matplotlib's smaller
#: default.
RC = {
...
#:            -- the earlier generation of these figures did this over the
#:            whole stationary half rather than a window.
MODES = {
    "timeavg": {
        "slug":   "time-averaged",
        "bins3":  30, "bins4": 24,
...
def load_record(path: Path, window: int, panels: int = 4) -> dict:
    ...
    with np.load(path) as z:
        ...
        # Figure 3's four stages: log-spaced over training, starting at
        # snapshot 0. The record is linearly spaced, so a
...
def main(argv=None) -> None:
    ...
    # Default: beside the record it came from. Pass out_dir=<dir> to collect the
    # paper's copies somewhere else instead.
