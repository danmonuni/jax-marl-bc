"""Grid-search heterogeneous capital productivity with i.i.d.-sampled kappas.

Sibling of ``runs/ks-wealth-calibration/sweep_lognormal.py`` (see that
directory's README for the full original writeup and Aldo's guidance): same
LogNormal(mu, sigma) design -- mean fixed to 1 via mu = -sigma^2/2, sigma
swept over a grid from 0 (homogeneous) to 1 (wide spread) -- but kappas here
are drawn as genuine i.i.d. samples from the distribution
(``np.random.default_rng(seed)``) rather than placed on the sibling's
deterministic regular-quantile mesh. That means the realized cross-sectional
*shape* of kappa (not just its target moments) now varies with the RNG seed
at fixed sigma, on top of the RL training's own seed variance.

Because a single draw at a given sigma no longer pins down "what that sigma
looks like", this experiment sweeps a list of seeds at every sigma point,
not just sigma itself. Each (sigma, seed) pair deterministically fixes both
the kappa draw and the RL run (``run.seed = seed``), and every cell in the
resulting grid is trained and kept -- no cell is dropped or treated as
preferred over another. results.csv and the comparison figure show the full
spread of outcomes across seeds at each sigma.

Usage (same config.yaml + CLI-dotlist convention as the sibling experiment):
    python sweep_lognormal_random.py
    python sweep_lognormal_random.py n_agents=100 device=cpu sigmas=[0.0,0.5] seeds=[0,1]
"""
