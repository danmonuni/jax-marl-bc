...
Reruns a single cell of the v3 lognormal-kappa sweep
(``runs/ks-wealth-lognormal-random-3/``) -- by default its ``sigma=0.0
...
Usage (same config.yaml + CLI-dotlist convention as the sibling experiments):
    ...
    python runs/ks-fig34-rerun/rerun_fig34.py device=cpu n_snapshots=4
    python runs/ks-fig34-rerun/rerun_fig34.py sigma=0.4 seed=3   # any sweep cell
    python runs/ks-fig34-rerun/rerun_fig34.py protocol.train.num_envs=8
...
class RunConfig:
    ...
    # Starting capital: the v3 initialization, as the sweep ran it.
...
def main() -> None:
    ...
    print("single-run protocol (= the v3 sweep cell; asserted by verify_protocol):")
